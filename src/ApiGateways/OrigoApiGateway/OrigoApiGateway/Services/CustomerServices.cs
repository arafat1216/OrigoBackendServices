    ...
    public class CustomerServices : ICustomerServices
    {
        ...
        public async Task<Partner> CreatePartnerAsync(Guid organizationId, Guid callerId)
        {
            try
            {
                ...
                var response = await HttpClient.PostAsJsonAsync($"{_options.ApiPath}/partner", partnerDto);
                if (!response.IsSuccessStatusCode)
        ...
        public async Task<IList<Partner>> GetPartnersAsync()
        {
            ...
            {
                bool customersOnly = true;
                var partnersFound = await HttpClient.GetFromJsonAsync<IList<PartnerDto>>($"{_options.ApiPath}/partner");
