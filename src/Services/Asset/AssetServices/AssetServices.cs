...
namespace AssetServices
{
    public class AssetServices : IAssetServices
    {
           ...
           Guid[]? label, bool? isActiveState, bool? isPersonal, DateTime? endPeriodMonth, string search, int page, int limit, CancellationToken cancellationToken)
        {
            try
            {
                var pagedAssetLifeCycles = await _assetLifecycleRepository.GetAssetLifecyclesAsync(customerId, userId, status, department, category, label, isActiveState, isPersonal, endPeriodMonth,search, page, limit, cancellationToken);
