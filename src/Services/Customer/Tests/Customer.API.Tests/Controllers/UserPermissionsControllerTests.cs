...
public class UserPermissionsControllerTests : IClassFixture<CustomerWebApplicationFactory<Startup>>
{
    ...
    public UserPermissionsControllerTests(CustomerWebApplicationFactory<Startup> factory,
    ...
    public async Task AssignUsersPermissions()
    {
        var requestBody = new NewUsersPermission
        {
            ...
            {
                new() { UserId = _userOneId, AccessList = new List<Guid> { _customerId }, Role = "EndUser" },
                new()
                {
                    UserId = _userTwoId,
        ...
        var read = await response.Content.ReadFromJsonAsync<UsersPermissions>();
    ...
    public async Task AssignUsersPermissions_CheckIfItHandlesErrors()
    {
        var requestBody = new NewUsersPermission
        {
            ...
            {
                new() { UserId = _userOneId, AccessList = new List<Guid> { _customerId }, Role = "EndUser" },
                new() { UserId = _userTwoId, AccessList = new List<Guid> { _customerId }, Role = "Headmaster" },
                new()
                {
                    UserId = Guid.Empty,
    ...
    public async Task AssignUsersPermissions_BadRequest_TryToAddTwoDiffrentRolesForSameUser()
    {
        var requestBody = new NewUsersPermission
        {
            UserPermissions = new List<NewUserPermissionDTO>
            {
                new() { UserId = _userTwoId, AccessList = new List<Guid> { _customerId }, Role = "EndUser" },
                new()
                {
                    UserId = _userTwoId,
                    AccessList = new List<Guid> { _customerId, _headDepartmentId },
    ...
    public async Task GetUserPermission_ChangeUserStatusIfInvited()
    {
        ...
        Assert.Equal(CustomerStatus.StartedOnboarding, organization?.Status);
    ...
    public async Task AssignUsersPermissions_AssignEndUserAdminRole()
    {
        var requestBody = new NewUsersPermission
        {
            UserPermissions = new List<NewUserPermissionDTO>
            {
                new() { UserId = _userOneId, AccessList = new List<Guid> { _customerId }, Role = "Admin" }
            }
        };
        var requestUri = "/api/v1/organizations/users/permissions";
        ...
        var response = await _httpClient.PutAsJsonAsync(requestUri, requestBody);
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        var userPermissions = await response.Content.ReadFromJsonAsync<UsersPermissions>();
