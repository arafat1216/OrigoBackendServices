...
public class OrganizationRepository : IOrganizationRepository
{
    ...
    public OrganizationRepository(CustomerContext customerContext, IFunctionalEventLogService functionalEventLogService,
        IMediator mediator)
    {
    ...
    public async Task<OrganizationPreferences> AddOrganizationPreferencesAsync(
    ...
    /// <inheritdoc />
    public async Task<IList<OrganizationUserCount>?> GetAllOrganizationsUsersCountAsync(Guid? partnerId,
        Guid[]? assignedToDepartment)
    {
        if (partnerId.HasValue)
        {
            return await _customerContext.Users.Include(u => u.Customer).ThenInclude(o => o.Partner)
                .Where(u => u.Customer.Partner != null && u.Customer.Partner.ExternalId == partnerId && !u.IsDeleted)
                .GroupBy(u => u.Customer.OrganizationId).Select(group => new OrganizationUserCount
                {
                    OrganizationId = group.Key,
                    Count = group.Count(u => u.UserStatus == UserStatus.Activated),
                    NotOnboarded = group.Count(u => u.UserStatus == UserStatus.Invited ||
                                                    u.UserStatus == UserStatus.OnboardInitiated ||
                                                    u.UserStatus == UserStatus.AwaitingConfirmation ||
                                                    u.UserStatus == UserStatus.NotInvited)
        ...
        {
            return await _customerContext.Users.Where(u => !u.IsDeleted).GroupBy(u => u.Customer.OrganizationId).Select(
                group => new OrganizationUserCount
                {
                    OrganizationId = group.Key,
                    Count = group.Count(u => u.UserStatus == UserStatus.Activated),
                    NotOnboarded = group.Count(u => u.UserStatus == UserStatus.Invited ||
    ...
    /// <inheritdoc />
    public async Task<OrganizationUserCount?> GetOrganizationUsersCountAsync(Guid customerId,
        Guid[]? assignedToDepartment, string[]? role)
    {
        if (assignedToDepartment != null)
        {
            return await _customerContext.Users.Include(c => c.Customer).Include(d => d.Department)
                .Where(user =>
                    user.Department != null && !user.IsDeleted && user.Customer.OrganizationId == customerId &&
                    assignedToDepartment.Contains(user.Department.ExternalDepartmentId))
                .GroupBy(a => a.Customer.OrganizationId).Select(group => new OrganizationUserCount
                {
                    OrganizationId = group.Key,
                    Count = group.Count(u => u.UserStatus != UserStatus.Deactivated &&
                                             u.UserStatus != UserStatus.OffboardCompleted &&
                                             u.UserStatus != UserStatus.OffboardOverdue),
                    NotOnboarded = group.Count(u => u.UserStatus == UserStatus.Invited ||
                                                    u.UserStatus == UserStatus.OnboardInitiated ||
                                                    u.UserStatus == UserStatus.AwaitingConfirmation ||
                                                    u.UserStatus == UserStatus.NotInvited)
        ...
        {
            if (role.Contains("CustomerAdmin") || role.Contains("PartnerAdmin") || role.Contains("SystemAdmin") ||
                role.Contains("GroupAdmin") || role.Contains("Admin") || role.Contains("PartnerReadOnlyAdmin"))
            {
                return await _customerContext.Users
                    .Where(user => user.Customer.OrganizationId == customerId && !user.IsDeleted)
                    .GroupBy(a => a.Customer.OrganizationId).Select(group => new OrganizationUserCount
                    {
                        OrganizationId = group.Key,
                        Count = group.Count(u => u.UserStatus == UserStatus.Activated),
    ...
    public async Task<IList<Organization>> GetOrganizationsAsync(bool asNoTracking,
        Expression<Func<Organization, bool>>? whereFilter = null, bool customersOnly = true, bool excludeDeleted = true,
        bool includeDepartments = false, bool includeAddress = false, bool includePartner = true)
    {
    ...
    public async Task<PagedModel<Organization>> GetPaginatedOrganizationsAsync(bool asNoTracking,
        CancellationToken cancellationToken, Expression<Func<Organization, bool>>? whereFilter = null,
        bool customersOnly = true, bool excludeDeleted = true, bool includeDepartments = false,
        bool includeAddress = false, bool includePartner = false, int page = 1, int limit = 25)
    {
        IQueryable<Organization> query = _customerContext.Set<Organization>();
    ...
    public async Task<Organization?> GetOrganizationAsync(Guid organizationId,
        Expression<Func<Organization, bool>>? whereFilter = null, bool customersOnly = true, bool excludeDeleted = true,
        bool includeDepartments = false, bool includeAddress = false, bool includePartner = true,
        bool includeLocations = false, bool asNoTracking = false)
    {
        IQueryable<Organization> query = _customerContext.Set<Organization>();
    ...
    {
        return await _customerContext.Organizations.FindAsync(id);
    }
    ...
    {
        return await _customerContext.Locations.FirstOrDefaultAsync(c => c.ExternalId == locationId);
    }

    public async Task<IList<Location>?> GetOrganizationAllLocationAsync(Guid organizationId)
    {
        var org = await _customerContext.Organizations.Include(x => x.Locations)
    ...
    public async Task<OrganizationPreferences> DeleteOrganizationPreferencesAsync(
        OrganizationPreferences organizationPreferences)
    {
        try
        {
    ...
    public async Task<Organization?> GetOrganizationByOrganizationNumber(string organizationNumber)
    {
        ...
        return await _customerContext.Organizations
            .Where(c => c.OrganizationNumber == organizationNumber && !c.IsDeleted).FirstOrDefaultAsync();
    }

    /// <inheritdoc />
    public async Task<User?> GetUserByEmailAddress(string emailAddress, bool includeCustomer = false,
        bool includeDepartment = false, bool includeUserPreference = false, bool asNoTracking = false)
    {
        ...
        return await query.Where(user => user.Email == emailAddress).FirstOrDefaultAsync();
    }
    ...
    public async Task<PagedModel<UserDTO>> GetAllUsersAsync(Guid customerId, string[]? role,
        Guid[]? assignedToDepartment, IList<int>? userStatus, bool asNoTracking, CancellationToken cancellationToken,
        string search = "", int page = 1, int limit = 25)
    {
        var query = _customerContext.Users.Include(u => u.Customer).Include(u => u.UserPreference)
            .Include(u => u.Department).Include(u => u.ManagesDepartments)
            .Where(u => u.Customer.OrganizationId == customerId && !u.IsDeleted).SelectMany(
                u => _customerContext.UserPermissions.Where(up => u.Id == up.User.Id).DefaultIfEmpty(),
                (u, up) => new UserDTO
                {
                    ...
                    UserPreference =
                        u.UserPreference == null
                            ? null
                            : new UserPreferenceDTO { Language = u.UserPreference.Language },
                    Id = u.UserId,
                    ...
                    ManagerOf = u.ManagesDepartments.Select(a =>
                            new ManagerOfDTO { DepartmentId = a.ExternalDepartmentId, DepartmentName = a.Name })
                        .ToList()
                });
        ...
        return await query.OrderBy(u => u.FirstName).PaginateAsync(page, limit, cancellationToken);
    }

    /// <inheritdoc />
    public async Task<User?> GetUserAsync(Guid organizationId, Guid userId, bool includeDepartment = false,
        bool includeUserPreference = false, bool asNoTracking = false)
    {
        var query = _customerContext.Users.Include(u => u.Customer)
    ...
    {
        return await _customerContext.Users.Where(u => u.UserId == userId).Include(u => u.Customer)
            .Include(u => u.UserPreference).Include(u => u.Department).Include(u => u.ManagesDepartments)
            .FirstOrDefaultAsync();
    }
    ...
    /// <inheritdoc />
    public async Task<IList<User>> GetUsersForCustomerAsync(Guid organizationId, bool asNoTracking,
    ...
    {
        IQueryable<Organization> query = _customerContext.Organizations.Where(a => a.OrganizationId == organizationId)
            .Include(d => d.Departments).ThenInclude(a => a.Managers);
        ...
        return await query.SelectMany(a => a.Departments).ToListAsync();
    }

    public async Task<PagedModel<Department>> GetPaginatedDepartmentsAsync(Guid organizationId, bool includeManagers,
        bool asNoTracking, CancellationToken cancellationToken, int page = 1, int limit = 25)
    {
        var query = _customerContext.Organizations.Where(a => a.OrganizationId == organizationId);

        if (includeManagers)
        {
            query = query.Include(d => d.Departments).ThenInclude(a => a.Managers);
        }
        else
        {
        ...
        return await query.SelectMany(a => a.Departments).PaginateAsync(page, limit, cancellationToken);
    }

    public async Task<Department?> GetDepartmentAsync(Guid organizationId, Guid departmentId)
    {
        return await _customerContext.Departments.Include(d => d.ParentDepartment).Include(m => m.Managers)
            .FirstOrDefaultAsync(p =>
                p.Customer.OrganizationId == organizationId && p.ExternalDepartmentId == departmentId);
    }
    ...
    {
        var query = _customerContext.Partners.Where(partner => !partner.IsDeleted && partner.ExternalId == partnerId);
    ...
    {
        return await _customerContext.Partners.AnyAsync(e => e.Organization.Id == organizationId);
    }

    /// <inheritdoc />
    public async Task<IList<Partner>> GetPartnersAsync(bool asNoTracking)
    {
        IQueryable<Partner> query = _customerContext.Partners.Where(partner => !partner.IsDeleted)
            .Include(o => o.Organization).OrderBy(o => o.Organization.Name);
    ...
    {
        return await _customerContext.Organizations.FirstOrDefaultAsync(a =>
            a.TechstepCustomerId == techstepCustomerId);
    }
    ...
    public async Task<IList<Guid>> GetOrganizationIdsForPartnerAsync(Guid partnerId)
    {
        var organizationList = await _customerContext.Organizations
            ...
            .Include(o => o.Partner).ThenInclude(p => p.Organization)
